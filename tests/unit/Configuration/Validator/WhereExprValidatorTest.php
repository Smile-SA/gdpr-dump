...
final class WhereExprValidatorTest extends TestCase
{
    ...
    public function testValidExpression(): void
    {
        ...
        (new WhereExprValidator())
            ->validate('email like "%@test.org" and created_at > date_sub(now(), interval 55 day)');
    }
    ...
    {
        $this->expectNotToPerformAssertions();
        (new WhereExprValidator())->validate('order_id in (select entity_id from order where status = "closed")');
    }
    ...
    public function testForbiddenStatement(): void
    {
        ...
        (new WhereExprValidator())->validate('drop database example');
    }
    ...
    {
        $this->expectException(InvalidQueryException::class);
        (new WhereExprValidator())->validate('1); select * from customer where (1');
    }
}
