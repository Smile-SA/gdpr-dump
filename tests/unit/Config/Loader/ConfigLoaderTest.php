...
use Smile\GdprDump\Configuration\Exception\FileLocatorException;
use Smile\GdprDump\Configuration\Loader\Loader;
use Smile\GdprDump\Configuration\Loader\ConfigLoader;
use Smile\GdprDump\Configuration\Loader\TemplateLocator;
...
final class ConfigLoaderTest extends TestCase
{
    ...
    // {
    //     $configuration = new Config(['version' => '2.0.0']);
    //     $configLoader = $this->createConfigLoader();
    //     $configLoader->load($configuration, $this->getResource('config/templates/test.yaml'));

    //     $expectedSubset = ['output' => '%env(DUMP_OUTPUT)%'];
    //     $this->assertArraySubset($expectedSubset, $configuration->get('dump'));

    //     $tablesConfig = $configuration->get('tables');
    ...
    //     $configLoader->load($configuration, 'not_exists.yaml');
    // }
    ...
    //     $configLoader->load($configuration, $this->getResource('config/templates/invalid_data.yaml'));
    // }
    ...
    //  */
    // private function createConfigLoader(): Loader
    // {
    //     $templatesDirectory = $this->getResource('config/templates');
    //     $eventDispatcherMock = $this->createMock(EventDispatcherInterface::class);

    //     return new ConfigLoader(new TemplateLocator($templatesDirectory), $eventDispatcherMock);
    // }
}
