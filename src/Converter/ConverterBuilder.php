...
final class ConverterBuilder
{
    ...
    public function build(ConverterConfig $definition): Converter
    {
        try {
            ...
            if ($converter instanceof IsInternal) {
                ...
                throw new ConverterBuildException(sprintf($message, $name, $converter->getAlternative()));
            }
    ...
    {
        if (!is_object($definitionCandidate) || !$definitionCandidate instanceof ConverterConfig) {
            throw new ConverterBuildException(sprintf('The parameter "%s" must be a converter.', $name));
        }
